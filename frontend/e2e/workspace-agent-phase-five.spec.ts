...
test("launcher is service-filtered and stays clear of mobile navigation", async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: "Open Ask SATHOS" })).toHaveCount(0);

  await installSyntheticApi(page, { enabledServices: ["CRM", "FINANCE", "B2BRAIN_AGENT"], permissions: [] });
  await page.reload();
  const launcher = page.getByRole("button", { name: "Open Ask SATHOS" });
...
test("drawer opens a clean chat, preserves an unsent draft, and creates no empty conversation", async ({ page }) => {
  ...
  await page.goto("/dashboard");
  await page.getByRole("button", { name: "Open Ask SATHOS" }).click();
  let drawer = page.getByRole("dialog", { name: "Ask SATHOS" });
  ...
  await drawer.getByLabel("Message Ask SATHOS").fill("My unsent question");
  await drawer.getByRole("button", { name: "Close Ask SATHOS" }).click();
  await page.getByRole("button", { name: "Open Ask SATHOS" }).click();
  drawer = page.getByRole("dialog", { name: "Ask SATHOS" });
  await expect(drawer.getByLabel("Message Ask SATHOS")).toHaveValue("My unsent question");
  ...
  drawer = page.getByRole("dialog", { name: "Ask SATHOS" });
  await expect(drawer.getByRole("heading", { name: "Start a new conversation" })).toBeVisible();
  await drawer.getByLabel("Message Ask SATHOS").fill("Clear this draft");
  await drawer.getByRole("button", { name: "New chat" }).click();
  await expect(drawer.getByLabel("Message Ask SATHOS")).toHaveValue("");
  await drawer.getByRole("link", { name: "Full workspace" }).click();
  await expect(page).toHaveURL(/\/agent$/);
  await expect(page.getByRole("heading", { name: "Ask SATHOS" })).toBeVisible();
...
test("desktop and tablet launcher drags without opening, snaps, and restores its scoped position", async ({ page }, testInfo) => {
  ...
  await page.mouse.up();
  await expect(page.getByRole("dialog", { name: "Ask SATHOS" })).toHaveCount(0);
  ...
  await page.reload();
  const restored = await page.getByRole("button", { name: "Open Ask SATHOS" }).boundingBox();
