...
test("guided WhatsApp Business Test Mode setup completes safely and responsively", async ({ page }) => {
  ...
  await page.route("**/api/v1/automation-bridge/connectors/*/whatsapp-setup/**", async (route) => {
    ...
    if (path.endsWith("/authorize")) return reply({ authorizationUrl: "/automation?section=connections&channel=whatsapp&wa_code=fake-approved&wa_state=synthetic-whatsapp-state-with-safe-entropy-123456&wa_connector=con-whatsapp-cloud-e2e" });
    ...
    if (path.endsWith("/reconnect")) { setupState = "AUTHORIZATION_STARTED"; ready = false; return reply({ authorizationUrl: "/automation?section=connections&channel=whatsapp&wa_code=fake-approved&wa_state=fresh-synthetic-whatsapp-state-with-safe-entropy-123&wa_connector=con-whatsapp-cloud-e2e" }); }
