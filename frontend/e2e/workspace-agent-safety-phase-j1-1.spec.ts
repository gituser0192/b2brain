...
test("confirmation actions send the signed preview decision", async ({ page }) => {
  ...
  await drawer.getByLabel("Message Ask SATHOS").fill("Add Synthetic Customer phone 9876543210");
...
test("verified zero and unavailable metrics stay distinct", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "Open Ask SATHOS" }).click();
  const drawer = page.getByRole("dialog", { name: "Ask SATHOS" });
  await drawer.getByLabel("Message Ask SATHOS").fill("Check my business");
  await drawer.getByRole("button", { name: "Send" }).click();
