...
test("renders verified bounded business records with accessible deep links", async ({ page }) => {
  ...
  await drawer.getByLabel("Message Ask B² Brain").fill("Find customer Synthetic Customer");
...
test("shows unavailable data without presenting a verified zero", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "Open Ask B² Brain" }).click();
  const drawer = page.getByRole("dialog", { name: "Ask B² Brain" });
  await drawer.getByLabel("Message Ask B² Brain").fill("Show overdue invoices");
  await drawer.getByRole("button", { name: "Send" }).click();
