...
test("Business Operating Agent alert visual baseline", async ({ page }) => {
  await installSyntheticApi(page);
  await page.route("**/api/v1/workspace-agent/brief", (route) => route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ success: true, data: { generatedAt: "2026-09-02T09:00:00.000Z", comparisonPeriod: "2026-08-31T18:30:00.000Z to 2026-09-01T18:30:00.000Z (Asia/Kolkata)", complete: false, ruleVersion: "j6-v1", executiveSummary: "1 verified priority needs review.", priorities: [{ key: "CRM:FOLLOW_UP:1", title: "Customer follow-up", reason: "2 follow-ups are overdue", service: "CRM", severity: "HIGH", date: "2026-09-01T09:00:00.000Z", evidence: "PENDING · due yesterday", href: "/crm", provenance: "ORGANIZATION_DATA", retrievedAt: "2026-09-02T09:00:00.000Z" }], changes: [], recommendations: [{ title: "Review customer follow-up", reason: "2 follow-ups are overdue", href: "/crm", action: null }], coverage: [{ source: "crm.follow_ups_due", service: "CRM", status: "VERIFIED", retrievedAt: "2026-09-02T09:00:00.000Z" }, { source: "finance.overdue_invoices", service: "FINANCE", status: "FORBIDDEN", retrievedAt: "2026-09-02T09:00:00.000Z" }], coverageSummary: { checked: 10, unavailable: 0, forbidden: 1, failed: 0, noData: 8, retrievedAt: "2026-09-02T09:00:00.000Z" }, mutationPerformed: false, externalCallPerformed: false } }) }));
  await page.goto("/agent");
  await expect(page.getByText("1 verified priority needs review.")).toBeVisible();
