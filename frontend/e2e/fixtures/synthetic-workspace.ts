...
export async function installSyntheticApi(page: Page, options: { authenticated?: boolean; restricted?: boolean; platformAdmin?: boolean; failServiceContext?: boolean; delayServiceContext?: number; failPlatformOverview?: boolean; delayPlatformOverview?: number; enabledServices?: string[]; permissions?: string[]; delayDashboard?: number; failDashboard?: boolean; emptyDashboard?: boolean; dashboardAlerts?: { type: string; count: number; label: string; view: string }[]; longIdentity?: boolean; delayCustomers?: number; failCustomers?: boolean; emptyCustomers?: boolean; delayProjects?: number; failProjects?: boolean; emptyProjects?: boolean; delayFinance?: number; failFinance?: boolean; emptyFinance?: boolean; delayEmployees?: number; failEmployees?: boolean; emptyEmployees?: boolean; richFinance?: boolean; richAutomation?: boolean; metaAutomation?: boolean; whatsappAutomation?: boolean } = {}) {
  ...
  await page.route("**/api/v1/**", async (route) => {
    ...
    if (clean === "/platform/overview" && options.platformAdmin) return json(route, { success: true, data: { organizations: [{ id: "org-e2e-safe", name: "E2E Safety Works", slug: "e2e-safety-works", status: "ACTIVE", createdAt: NOW, activeMemberCount: 1, enabledServiceIds: ALL_SERVICES.map((code) => `def-${code}`), owner: session.user, plan: null }], services: ALL_SERVICES.map((code) => ({ id: `def-${code}`, code, name: code.replaceAll("_", " "), description: "Synthetic service", status: "ACTIVE", enabledOrganizationCount: 1, maturity: maturity(code) })), invitations: [], plans: [] } });
