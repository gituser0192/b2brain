...
function fixture(path: string, method: string, session: typeof ownerSession) {
  ...
  if (clean === "/services/context") return { success: true, data: { catalog: [], enabledServices: SERVICES.map((code) => ({ id: `svc-${code}`, enabledAt: NOW, service: { id: `def-${code}`, code, name: code.replaceAll("_", " "), description: "Synthetic enabled service", iconKey: null, routePath: null, featureFlags: [] } })) } };
