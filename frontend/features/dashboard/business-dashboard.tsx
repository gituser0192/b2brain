...
interface Metrics {
  ...
  activeProjects: number; overdueTasks: number; activeEmployees: number;
  invoiced: number; received: number; outstanding: number; expenses: number; netCash: number;
...
interface DashboardData { periodDays: number | null; enabledServices: string[]; currency: string; metrics: Metrics; alerts: Alert[] }
  ...
  return <div className="business-dashboard">
    ...
    {dashboardQuery.isLoading || !metrics ? <div className="dashboard-data-loader"><span className="spinner dark" />Calculating your business…</div> : <>
      ...
      <section className="executive-metrics">
        {has("FINANCE") && <article className="primary"><div><span>Net cash</span><strong>{money(metrics.netCash)}</strong><small>{money(metrics.received)} received · {money(metrics.expenses)} expenses</small></div><i>₹</i></article>}
