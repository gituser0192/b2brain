...
export function SchoolWorkspace() {
  ...
  const [data, setData] = useState<Payload["data"]>({
    academicYears: [],
    metrics: { academicYears: 0, classes: 0, sections: 0, students: 0 },
  });
  ...
  const canManage =
    session?.membership.permissions.includes("SCHOOL_MANAGE") ?? false;
  const classes = data.academicYears.flatMap((item) =>
    item.classes.map((value) => ({ ...value, year: item.name })),
  );

  const load = useCallback(async () => {
    try {
      const response = await authorizedRequest<Payload>("/school");
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      setMode(null);
      setNotice("School structure saved.");
      await load();
    } catch (reason) {
      setError(
        reason instanceof ApiError
          ? reason.message
          : "Unable to save school structure.",
      );
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div className="school-workspace">
      <header className="project-heading">
        ...
        {canManage && (
          <div>
            <button onClick={() => setMode("year")}>+ Academic year</button>
            <button
              disabled={!data.academicYears.length}
              onClick={() => {
                setSchoolClass({
                  ...schoolClass,
            ...
            <button
              ...
              onClick={() => {
                setSection({ ...section, classId: classes[0]?.id ?? "" });
                setMode("section");
              }}
            >
              + Section
      ...
      <section className="school-metrics">
        <article>
          <span>Academic years</span>
          <strong>{data.metrics.academicYears}</strong>
        </article>
        <article>
          <span>Classes</span>
          <strong>{data.metrics.classes}</strong>
        </article>
        <article>
          <span>Sections</span>
          <strong>{data.metrics.sections}</strong>
        </article>
        <article>
      ...
      {!data.academicYears.length ? (
        <section className="project-empty">
          <span>⌂</span>
          <h3>No academic structure configured</h3>
          <p>
            Create your first academic year. No sample classes, students,
            teachers, attendance, or timetable records are generated.
          </p>
          {canManage && (
          ...
          {data.academicYears.map((item) => (
            <article key={item.id}>
              <header>
                <div>
                  <small>
                    {item.isCurrent ? "Current academic year" : "Academic year"}
                  </small>
                  <h3>{item.name}</h3>
                  <span>
                ...
                <b>{item.classes.length} classes</b>
              </header>
              {!item.classes.length ? (
                <p>No classes created for this year.</p>
              ) : (
                      ...
                      <p>
                        {value.sections.length
                          ? value.sections
                              .map(
                                (entry) =>
                                  `${entry.name}${entry.room ? ` · Room ${entry.room}` : ""}`,
                              )
                              .join("  |  ")
                          : "No sections"}
                      </p>
                ...
                <>
                  ...
                  <label>
                    ...
                    <input
                      ...
                      value={year.startsOn}
                      onChange={(e) =>
                        setYear({ ...year, startsOn: e.target.value })
                      }
                    />
                  </label>
                  <label>
                    ...
                    <input
                      ...
                      value={year.endsOn}
                      onChange={(e) =>
                        setYear({ ...year, endsOn: e.target.value })
                      }
                    />
                  </label>
                  <label className="school-check">
                    <input
                      ...
                      onChange={(e) =>
                        setYear({ ...year, isCurrent: e.target.checked })
                      }
                    />{" "}
                ...
                <>
                  ...
                  <label>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setSchoolClass({ ...schoolClass, code: e.target.value })
                      }
                    />
                  </label>
                  <label>
                ...
                <>
                  ...
                  <label>
                    ...
                    <input
                      value={section.room}
                      onChange={(e) =>
                        setSection({ ...section, room: e.target.value })
                      }
                    />
                  </label>
                  <label>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setSection({ ...section, capacity: e.target.value })
                      }
                    />
                  </label>
                </>
