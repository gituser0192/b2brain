...
type Payload = { success: true; data: { academicYears: AcademicYear[]; metrics: { academicYears: number; classes: number; sections: number; students: number } } };

const today = new Date().toISOString().slice(0, 10);
export function SchoolWorkspace() {
  const { session, authorizedRequest } = useAuth();
  const [data, setData] = useState<Payload["data"]>({ academicYears: [], metrics: { academicYears: 0, classes: 0, sections: 0, students: 0 } });
  ...
  return <div className="school-workspace">
    ...
    <section className="school-metrics"><article><span>Academic years</span><strong>{data.metrics.academicYears}</strong></article><article><span>Classes</span><strong>{data.metrics.classes}</strong></article><article><span>Sections</span><strong>{data.metrics.sections}</strong></article><article><span>Active students</span><strong>{data.metrics.students}</strong></article></section>
    {!!data.academicYears.length&&<StudentDirectory academicYears={data.academicYears} canManage={canManage} onChanged={load}/>}
