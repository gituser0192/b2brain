...
export function MetaLeadSetup({ connectorId, canManage = true }: { connectorId: string; canManage?: boolean }) {
  ...
  async function connect() { await run(async () => { const start = await authorizedRequest<{ success: true; data: { authorizationUrl: string } }>(`/automation-bridge/connectors/${connectorId}/meta/authorize`, { method: "POST", body: JSON.stringify({ returnPath: "/automation?section=connections&channel=meta" }) }); router.push(start.data.authorizationUrl); }); }
  ...
  return <section className="meta-lead-setup" aria-labelledby="meta-setup-title">
    ...
    <div className="meta-setup-actions">
      <button disabled={!canManage || busy} onClick={() => void connect()}>Connect Meta</button>
      {pages.length > 0 && <label>Authorized Page<select disabled={!canManage} value={selectedPage} onChange={event => setSelectedPage(event.target.value)}><option value="">Choose Page</option>{pages.map(page => <option key={page.id} value={page.id}>{page.name}</option>)}</select><button disabled={!canManage || busy || !selectedPage} onClick={() => void choosePage()}>Use Page</button></label>}
      {forms.length > 0 && <fieldset disabled={!canManage}><legend>Permitted lead forms</legend>{forms.map(form => <label key={form.id}><input type="checkbox" checked={selectedForms.includes(form.id)} onChange={event => setSelectedForms(current => event.target.checked ? [...current, form.id] : current.filter(id => id !== form.id))}/>{form.name}</label>)}<button disabled={busy || selectedForms.length === 0} onClick={() => void chooseForms()}>Save forms</button></fieldset>}
      <button disabled={!canManage || busy || !status?.forms.length} onClick={() => void post("verify", "Synthetic subscription verified.")}>Verify subscription</button>
      <button disabled={!canManage || busy || !status?.subscriptionVerified} onClick={() => void post("test", "Synthetic lead passed without creating CRM data.")}>Run synthetic test</button>
      <button disabled title="Production activation is unavailable in Test Mode">Activate</button>
      <button disabled={!canManage || busy} onClick={() => void post("reconnect", "Reconnect authorization started.")}>Reconnect</button>
      <button disabled={!canManage || busy} onClick={() => { if (window.confirm("Disconnect Meta Lead Ads? Historical CRM records will be preserved.")) void post("disconnect", "Meta Lead Ads disconnected. Historical records were preserved."); }}>Disconnect</button>
    </div>
