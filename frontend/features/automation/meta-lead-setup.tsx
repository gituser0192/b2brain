...
export function MetaLeadSetup({ connectorId, canManage = true }: { connectorId: string; canManage?: boolean }) {
  ...
  async function connect() { await run(async () => { const start = await authorizedRequest<{ success: true; data: { authorizationUrl: string } }>(`/automation-bridge/connectors/${connectorId}/meta/authorize`, { method: "POST", body: JSON.stringify({ returnPath: "/automation?section=connections" }) }); router.push(start.data.authorizationUrl); }); }
