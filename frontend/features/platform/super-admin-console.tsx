...
type Confirmation = { kind: "revoke"; invitation: PlatformInvitation } | { kind: "approve" | "suspend" | "reactivate" | "remove"; organization: PlatformOrganization };
      ...
      {section === "organizations" && !loadFailed && <section className={`platform-organizations ${selected || organizationNotFound ? "show-detail" : ""}`}>
        ...
        <div className="organization-detail" aria-live="polite">
          {organizationNotFound ? <div className="platform-empty"><h2>Organization not found</h2><p>The selected organization is unavailable or no longer accessible.</p><Link href="/super-admin?section=organizations">Back to organizations</Link></div> : !selected ? <div className="platform-empty"><h2>Select an organization</h2><p>Choose a customer account to review access without changing your own organization membership.</p></div> : <>
            ...
            <section className="organization-access-actions"><div><p>Account access</p><h3>Login and workspace access</h3><span>{selected.status === "PENDING_APPROVAL" ? "The owner has completed signup. Approve to enable login; plans and services are managed separately." : "Suspension blocks access without deleting business data. Restoring access does not change plans or services."}</span></div>{selected.owner?.isPlatformAdmin ? <p>Protected Super Admin organization</p> : <div>{selected.status === "ACTIVE" ? <button type="button" onClick={(event) => openConfirmation({ kind: "suspend", organization: selected }, event.currentTarget)}>Suspend access</button> : <button type="button" onClick={(event) => openConfirmation({ kind: selected.status === "PENDING_APPROVAL" ? "approve" : "reactivate", organization: selected }, event.currentTarget)}>{selected.status === "PENDING_APPROVAL" ? "Approve login" : "Restore access"}</button>}<button type="button" className="remove" onClick={(event) => openConfirmation({ kind: "remove", organization: selected }, event.currentTarget)}>Remove account</button></div>}</section>
          </>}
        </div>
      </section>}
