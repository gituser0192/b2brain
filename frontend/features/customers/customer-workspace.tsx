...
interface FollowUpItem { id: string; title: string; description: string | null; dueAt: string; status: "PENDING" | "COMPLETED" | "CANCELED"; customer: { id: string; displayName: string; email: string | null; phone: string | null }; assignedTo: { id: string; firstName: string; lastName: string | null }; }
interface FollowUpResponse { success: true; data: { items: FollowUpItem[]; metrics: { pending: number; overdue: number; dueToday: number; completed: number } }; }
...
export function CustomerWorkspace() {
  ...
  return <div className="crm-workspace">
    ...
    {view === "followups" && <>
      ...
      <section className="crm-followup-panel"><header><div><strong>Work queue</strong><span>{followUps.length} follow-ups shown</span></div><label><input type="checkbox" checked={myFollowUps} onChange={(event) => setMyFollowUps(event.target.checked)} /> Assigned to me</label></header>
        {loading ? <div className="roles-loading"><span className="spinner dark" /> Loading follow-ups...</div> : followUps.length === 0 ? <div className="crm-empty"><div><span>✓</span></div><h3>No follow-ups here</h3><p>Schedule a follow-up inside a customer profile and it will appear in this work queue.</p></div> : <div className="crm-followup-list">{followUps.map((item) => { const overdue = item.status === "PENDING" && new Date(item.dueAt) < new Date(); return <article key={item.id}><div><span className={`customer-status ${overdue ? "overdue" : item.status.toLowerCase()}`}>{overdue ? "OVERDUE" : item.status}</span><strong>{item.title}</strong><p>{item.customer.displayName} · Assigned to {item.assignedTo.firstName} {item.assignedTo.lastName ?? ""}</p></div><time>{new Intl.DateTimeFormat("en", { dateStyle: "medium", timeStyle: "short" }).format(new Date(item.dueAt))}</time><div>{item.customer.phone && <a href={`tel:${item.customer.phone}`}>Call</a>}{item.customer.email && <a href={`mailto:${item.customer.email}`}>Email</a>}{canManageFollowUps && item.status === "PENDING" && <button onClick={() => void completeFollowUp(item)}>Complete</button>}</div></article>; })}</div>}
      </section>
    </>}
