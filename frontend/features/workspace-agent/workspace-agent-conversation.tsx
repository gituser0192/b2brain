...
function AgentAnswer({ item, compact, loading, onNavigate, onSection, onConfirm }: {
  item: AgentItem; compact: boolean; loading: boolean; onNavigate?: (view: string) => void;
  onSection: (section: AgentSection) => void; onConfirm: (token: string, decision: "CONFIRM" | "CANCEL") => void;
}) {
  ...
  return <div className="workspace-agent-answer"><header><span>✦</span><strong>Ask B² Brain</strong></header><p>{output.answer}</p>
    ...
    {output.confirmation && <div className="agent-confirmation"><strong>Review required</strong>{output.confirmation.preview.name && <p>Name: {output.confirmation.preview.name}</p>}{output.confirmation.preview.phone && <p>Phone: {output.confirmation.preview.phone}</p>}{output.confirmation.preview.category && <p>Category: {output.confirmation.preview.category.replaceAll("_", " ")}</p>}<div><button type="button" disabled={loading} onClick={() => onConfirm(output.confirmation!.token, "CONFIRM")}>Confirm</button><button type="button" disabled={loading} onClick={() => onConfirm(output.confirmation!.token, "CANCEL")}>Cancel</button></div></div>}
...
export function WorkspaceAgentConversation({ items, loading, compact, prompts = defaultPrompts, onSend, onNavigate, onSection, onConfirm }: {
  items: AgentItem[]; loading: boolean; compact: boolean; onSend: (message: string) => void;
  prompts?: string[];
  onNavigate?: (view: string) => void; onSection: (section: AgentSection) => void; onConfirm: (token: string, decision: "CONFIRM" | "CANCEL") => void;
}) {
  return <div className="workspace-agent-thread" aria-live="polite">{!items.length && !loading ? <div className="workspace-agent-welcome"><div>✦</div><h3>{compact ? "Start a new conversation" : "How can I help your business?"}</h3><p>{compact ? "Ask about your business health, finances, customers or priorities." : "I can analyse permitted data, explain B² Brain and perform explicit low-risk actions."}</p><div>{prompts.map((prompt) => <button key={prompt} onClick={() => onSend(prompt)}>{prompt}</button>)}</div></div> : items.map((item) => <article key={item.id} className="workspace-agent-exchange"><div className="workspace-user-message"><strong>You</strong><p>{item.message}</p></div><AgentAnswer item={item} compact={compact} loading={loading} onNavigate={onNavigate} onSection={onSection} onConfirm={onConfirm} /></article>)}{loading && <div className="workspace-agent-thinking" role="status"><span className="spinner dark" />Ask B² Brain is checking permitted data…</div>}</div>;
}
