...
export function WorkspaceAgentConversation({ items, loading, compact, prompts = defaultPrompts, onSend, onNavigate, onSection }: {
  ...
  return <div className="workspace-agent-thread" aria-live="polite">{!items.length && !loading ? <div className="workspace-agent-welcome"><div>✦</div><h3>{compact ? "Start a new conversation" : "How can I help your business?"}</h3><p>{compact ? "Ask about your business health, finances, customers or priorities." : "I can analyse permitted data, explain B² Brain and perform explicit low-risk actions."}</p><div>{prompts.map((prompt) => <button key={prompt} onClick={() => onSend(prompt)}>{prompt}</button>)}</div></div> : items.map((item) => <article key={item.id} className="workspace-agent-exchange"><div className="workspace-user-message"><strong>You</strong><p>{item.message}</p></div><AgentAnswer item={item} compact={compact} onNavigate={onNavigate} onSection={onSection} /></article>)}{loading && <div className="workspace-agent-thinking" role="status"><span className="spinner dark" />Ask B² Brain is checking permitted data…</div>}</div>;
}
