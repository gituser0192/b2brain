...
const availableValue = (value: number | null, availability?: string) => value !== null ? value : availability === "FORBIDDEN" ? "No access" : availability === "FAILED" ? "Unavailable" : "Not available";
...
export function BusinessBriefView({ brief, onNavigate }: {
  ...
  return <section className="agent-management-view">
    <header><div><p>Daily business brief</p><h3>{brief.meaningful ? "What needs your attention today" : "No meaningful changes need attention"}</h3><span>{brief.period} · Calculated {new Intl.DateTimeFormat("en-IN", { dateStyle: "medium", timeStyle: "short" }).format(new Date(brief.calculatedAt))}</span></div><strong className="agent-health-score">{brief.health.score === null ? "Not available" : brief.health.score}<small>Health / 100</small></strong></header>
    ...
    <div className="workspace-metric-grid"><article><span>New customers today</span><strong>{availableValue(brief.activity.newCustomers, brief.availability?.newCustomers)}</strong></article><article><span>Overdue follow-ups</span><strong>{availableValue(brief.activity.overdueFollowUps, brief.availability?.overdueFollowUps)}</strong></article><article><span>Overdue tasks</span><strong>{availableValue(brief.activity.overdueTasks, brief.availability?.overdueTasks)}</strong></article><article><span>At-risk projects</span><strong>{availableValue(brief.activity.atRiskProjects, brief.availability?.atRiskProjects)}</strong></article></div>
