...
export function PublicQuotation({token}:{token:string}) {
  ...
  return <main className="public-quote-shell"><article className="public-quote">
    <header><div><span>SATHOS · COMMERCIAL PROPOSAL</span><h1>{quote.organization.name}</h1><p>Quotation {quote.quotationNumber}</p></div><div><b>{quote.status}</b><small>Valid until {new Date(quote.validUntil).toLocaleDateString()}</small></div></header>
    ...
    <footer><span>Generated securely by SATHOS</span><button onClick={()=>window.print()}>Print / Save PDF</button></footer>
  </article></main>;
}
