...
export const requireProviderPermission = (permission: string): RequestHandler => async (request, _response, next) => {
  ...
  if (!provider || !request.auth.permissions.includes(permission)) return next(new AppError(403, "SATHOS service-desk access is required.", "PROVIDER_ACCESS_REQUIRED"));
  next();
};
...
export const requireProviderSensitiveCompletion: RequestHandler = (request, _response, next) => {
  ...
  next(new AppError(403, "Sensitive completion requires SATHOS approval authority.", "PROVIDER_APPROVAL_REQUIRED"));
};
