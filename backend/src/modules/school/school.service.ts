...
export class SchoolService {
  ...
  private async requestCoverageApproval(organizationId:string,userId:string,date:string,absentTeacherIds:string[]){
    ...
    await prisma.$transaction(async tx=>{
      ...
      await tx.approvalRequest.create({data:{organizationId,serviceCode:"SCHOOL",actionCode:"APPLY_SCHOOL_COVERAGE_PLAN",title:`Approve substitute plan for ${date}`,description:`SATHOS prepared ${assignments.length} substitute assignment${assignments.length===1?"":"s"} after ${absentTeacherIds.length} teacher absence${absentTeacherIds.length===1?" was":"s were"} recorded.`,riskLevel:"MEDIUM",sourceType:"AUTOMATION_POLICY_EXECUTION",sourceId:result.execution.id,requestedById:userId,dueAt:new Date(Date.now()+4*60*60*1000),context:{date,assignments}}});
