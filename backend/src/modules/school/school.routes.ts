...
import { academicYearSchema, schoolClassSchema, schoolSectionSchema, schoolStudentSchema, schoolStudentUpdateSchema, type AcademicYearInput, type SchoolClassInput, type SchoolSectionInput, type SchoolStudentInput, type SchoolStudentUpdateInput } from "./school.validation.js";
...
schoolRouter.post("/students", requirePermission("SCHOOL_MANAGE"), validateBody(schoolStudentSchema), async (request, response) => { const context = auth(request); response.status(201).json(success(await service.createStudent(context.organizationId, context.userId, request.body as SchoolStudentInput), "Student admitted.")); });
schoolRouter.put("/students/:id", requirePermission("SCHOOL_MANAGE"), validateBody(schoolStudentUpdateSchema), async (request, response) => { const context = auth(request); response.json(success(await service.updateStudent(context.organizationId, context.userId, String(request.params.id), request.body as SchoolStudentUpdateInput), "Student updated.")); });
schoolRouter.delete("/students/:id", requirePermission("SCHOOL_MANAGE"), async (request, response) => { const context = auth(request); response.json(success(await service.archiveStudent(context.organizationId, context.userId, String(request.params.id)), "Student archived.")); });
