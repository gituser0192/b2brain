...
export const academicYearSchema = z
  ...
  .refine((value) => value.endsOn > value.startsOn, {
    message: "Academic year must end after it starts.",
    path: ["endsOn"],
  });
...
export const schoolSectionSchema = z
  .object({
...
const optionalText = (max: number) =>
  z
    ...
    .or(z.literal(""))
    .transform((value) => value || null);
export const schoolStudentSchema = z
  .object({
...
export const schoolStudentUpdateSchema = z
  .object({
    firstName: z.string().trim().min(2).max(100),
    lastName: optionalText(100),
    dateOfBirth: z
...
export const schoolSubjectSchema = z
  ...
  .strict();
export const schoolTeacherSchema = z
  .object({
    firstName: z.string().trim().min(2).max(100),
    lastName: optionalText(100),
    email: z
    ...
    maxPeriodsPerWeek: z.number().int().min(1).max(70),
  })
  .strict();
export const schoolTeacherAssignmentSchema = z
  .object({
  ...
  .strict();
export const schoolAttendanceQuerySchema = z
  .object({ date: z.string().date() })
  .strict();
      ...
      .array(
        z
          .object({
            ...
            remarks: optionalNullableText(300),
          })
          .strict(),
      )
      .min(1)
...
export const schoolTimetableQuerySchema = z
  .object({
  ...
  .strict();
export const schoolTimetableEntrySchema = z
  .object({
...
export const schoolSubstituteQuerySchema = z
  .object({ date: z.string().date() })
  .strict();
export const schoolSubstituteSchema = z
  .object({
    timetableEntryId: z.string().uuid(),
    attendanceDate: z.string().date(),
    substituteTeacherId: z.string().uuid(),
    notes: optionalNullableText(300),
  })
  .strict();
export const schoolImportConfirmSchema = z
...
export type SchoolStudentUpdateInput = z.infer<
  typeof schoolStudentUpdateSchema
>;
...
export type SchoolTeacherAssignmentInput = z.infer<
  typeof schoolTeacherAssignmentSchema
>;
...
export type SchoolTimetableEntryInput = z.infer<
  typeof schoolTimetableEntrySchema
>;
export type SchoolSubstituteInput = z.infer<typeof schoolSubstituteSchema>;
export type SchoolImportConfirmInput = z.infer<
  typeof schoolImportConfirmSchema
>;
