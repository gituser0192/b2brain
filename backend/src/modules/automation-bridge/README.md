...
## Guided WhatsApp Business connection (Phase G)

The authenticated Automation workspace provides a guided WhatsApp Business Test Mode setup for `WHATSAPP` / `META_WHATSAPP_CLOUD` connectors. It reuses the durable authorization-state table: a cryptographically random state is stored only as a SHA-256 hash for ten minutes, bound to the active user, membership, organization and connector, then claimed atomically once. Only safe relative `/automation` return paths are accepted.

The deterministic provider is entirely local and exposes bounded synthetic WABA and Phone Number ID choices. IDs selected by the browser must belong to the provider-returned hierarchy. Temporary runtime credentials are encrypted with the existing AES-256-GCM helper, are never returned or audited, and are cleared on disconnect. The unique Phone Number ID constraint prevents one number mapping to multiple connectors or organizations.

The default Test Connection is non-mutating: it checks synthetic ownership, scopes, number mapping and inbound readiness without calling Meta, sending messages, creating CRM records or invoking AI. A successful check leaves the connector `DRAFT` with a Test Ready marker; it never makes the connector production-active. Disconnect invalidates incomplete authorization and clears connector credentials and mappings while preserving CRM, integration-event and audit history.

Real activation still requires Meta Embedded Signup or an equivalent verified authorization flow, approved WhatsApp permissions, durable token lifecycle management, WABA/number ownership verification, real webhook subscription, compliance review, operational monitoring and deliberate production switch enablement. Phase G enables none of those and contains no real Meta identifiers or credentials.
