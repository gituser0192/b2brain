        ...
        if (approval.sourceType === "COLLECTION_AGENT_RUN") {
          ...
          if (input.decision === "APPROVE") {
            ...
            for (const item of context.assignments) {
              ...
              await transaction.schoolSubstituteAssignment.upsert({ where: { timetableEntryId_attendanceDate: { timetableEntryId: entry.id, attendanceDate } }, update: { substituteTeacherId: teacher.id, status: "ASSIGNED", notes: "Approved from the SATHOS automated coverage plan.", updatedById: actorUserId }, create: { organizationId, timetableEntryId: entry.id, attendanceDate, absentTeacherId: entry.teacherId, substituteTeacherId: teacher.id, notes: "Approved from the SATHOS automated coverage plan.", createdById: actorUserId, updatedById: actorUserId } });
            }
          }
