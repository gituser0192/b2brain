...
export const approveVoiceCall: RequestHandler = async (request, response) => { const context = auth(request); await service.approve(context.organizationId, context.userId, id(request.params.id)); response.json(success({}, "Voice call approved.")); };
