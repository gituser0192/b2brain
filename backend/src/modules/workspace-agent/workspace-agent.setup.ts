...
export async function assessWorkspaceSetup(context: WorkspaceAgentContext, request: string) {
  ...
  if (/(enable|disable).*(service)|service.*(enable|disable)/i.test(request)) return { answer: "I cannot enable or disable organization services. Contact your B² Brain administrator.", refused: true, areas: [], steps: [] };
