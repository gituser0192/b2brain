...
export type WorkspaceRoute = { intent: WorkspaceAgentIntent; path: ProcessingPath; aiRequired: boolean; confidence: RouteConfidence; normalizedRequest: string; ambiguity: "NONE" | "CLARIFICATION_REQUIRED"; candidateCapability?: string; missingRequiredFields?: string[]; parsedArguments?: Record<string, string>; clarification?: { question: string; choices: { label: string; request: string }[] }; resolvedDate?: { expression: string; date: string }; provenance: "ORGANIZATION_DATA" | "CALCULATION" | "GENERAL_KNOWLEDGE" | "INFERENCE"; toolName?: string; toolInput?: { query?: string; limit?: number; id?: string; period?: "THIS_MONTH" }; toolNames?: string[]; writeToolName?: "crm.follow_up_create" | "projects.task_create" | "leads.assign"; writeInput?: Record<string, string | number> };
...
export function routeWorkspaceRequest(message: string): WorkspaceRoute {
  ...
  const followUp = value.match(/create (?:a )?follow.?up (?:for )?(?:customer )?([0-9a-f-]{36}) (?:titled?|called) [“"]?(.+?)[”"]? (?:on|at|due) (\d{4}-\d{2}-\d{2}t\S+)/i);
  if (followUp) return result(value, "BUSINESS_READ", "WRITE_ACTION", { writeToolName: "crm.follow_up_create", writeInput: { customerId: followUp[1]!, title: followUp[2]!, dueAt: followUp[3]! } });
  const task = value.match(/create (?:a )?task [“"]?(.+?)[”"]? (?:in|for) (?:project )?([0-9a-f-]{36})(?: (?:due|on) (\d{4}-\d{2}-\d{2}t\S+))?/i);
  if (task) return result(value, "BUSINESS_READ", "WRITE_ACTION", { writeToolName: "projects.task_create", writeInput: { title: task[1]!, projectId: task[2]!, ...(task[3] ? { dueAt: task[3] } : {}) } });
  const assign = value.match(/assign (?:lead|inquiry) ([0-9a-f-]{36}) to (?:employee )?([0-9a-f-]{36})/i);
  if (assign) return result(value, "BUSINESS_READ", "WRITE_ACTION", { writeToolName: "leads.assign", writeInput: { inquiryId: assign[1]!, employeeId: assign[2]!, reason: "Assigned through Ask B² Brain after explicit confirmation", responseTimeMinutes: 1440 } });
  ...
  if (/(help me set up|agent setup|set up my business|setup.*help|setup.*madad|what.*missing.*setup|crm.*setup|finance.*setup|whatsapp.*(?:connect|setup)|team member.*add|business.*setup|password|otp|access token|app secret|verify token|encryption key|signing secret|webhook secret|(?:enable|disable).*(?:service))/i.test(value)) return result(value, "SETUP_GUIDANCE", "PRODUCT_HELP");
