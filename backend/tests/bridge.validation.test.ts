...
describe("automation bridge validation", () => {
  ...
  it("accepts the null external reference sent by the connector form", () => expect(connectorSchema.parse({ name: "Website Contact Form", type: "WEBSITE", provider: "SATHOS Hosted Form", externalAccountRef: null, status: "ACTIVE", mode: "ASSISTED" })).toMatchObject({ externalAccountRef: null, type: "WEBSITE" }));
