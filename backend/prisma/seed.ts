...
const permissions = [
  ...
  { code: "SCHOOL_VIEW", name: "View school operations", description: "View academic years, classes, sections, students, staff, attendance, and timetables" },
  { code: "SCHOOL_MANAGE", name: "Manage school operations", description: "Configure academic years, classes, sections, students, staff, attendance, and timetables" },
...
function schoolPermissions(roleCode: string) { return roleCode==="ORGANIZATION_MEMBER"?["SCHOOL_VIEW"] as const:["SCHOOL_VIEW","SCHOOL_MANAGE"] as const; }
...
async function seed() {
  ...
  await prisma.service.upsert({where:{code:"SCHOOL"},update:{name:"B² School Management",description:"Academic structure, students, teachers, attendance, timetables, fees, communication, and controlled school automation.",status:"ACTIVE",iconKey:"S",routePath:"/dashboard?view=school",sortOrder:13,archivedAt:null},create:{code:"SCHOOL",name:"B² School Management",description:"Academic structure, students, teachers, attendance, timetables, fees, communication, and controlled school automation.",status:"ACTIVE",iconKey:"S",routePath:"/dashboard?view=school",sortOrder:13}});
